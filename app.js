    ...
    Papa.parse(CSV_URL, {
      ...
      chunk: (chunk) => {
        ...
        for (const r of rows) {
          ...
          const hasOHLC = isFinite(r.Open) && isFinite(r.High) && isFinite(r.Low) && isFinite(r.Close);
...
function buildSlicer() {
  ...
  chk.addEventListener('change', () => { state.showVolume = chk.checked; plotSelected(); });
}
...
function plotSelected() {
  ...
  el('dateRange').textContent = (first && last)
...
(async function init() {
  try {
    ...
    if (state.tickers.length) selectTicker(state.tickers[0]);
  } catch (err) {
    console.error(err);
